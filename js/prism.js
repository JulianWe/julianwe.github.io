...
var Prism = (function (_self) {
	...
	var _ = {
		...
		util: {
			...
			/**
			 * Returns the Prism language of the given element set by a `sourceCodexxxx` or `lang-xxxx` class.
			 *
			...
			/**
			 * Sets the Prism `sourceCodexxxx` class of the given element.
			 *
			 * @param {Element} element
			...
			setLanguage: function (element, language) {
				// remove all `sourceCodexxxx` classes
				// (this might leave behind a leading space)
				element.className = element.className.replace(RegExp(lang, 'gi'), '');

				// add the new `sourceCodexxxx` class
				// (using `classList` will automatically clean up spaces for us)
				element.classList.add('sourceCode' + language);
			},
		 ...
		 * It fetches all the elements that have a `.sourceCodexxxx` class and then calls {@link Prism.highlightElement} on
		 * each one of them.
		 *
		...
		/**
		 * Fetches all the descendants of `container` that have a `.sourceCodexxxx` class and then calls
		 ...
		 *
		 * @param {ParentNode} container The root element, whose descendants that have a `.sourceCodexxxx` class will be highlighted.
		...
		highlightAllUnder: function (container, async, callback) {
			var env = {
				callback: callback,
				container: container,
				selector: 'code[class*="sourceCode"], [class*="sourceCode"] code, code[class*="sourceCode"], [class*="sourceCode"] code'
			};
		 ...
		 * It must have a class of `sourceCodexxxx` to be processed, where `xxxx` is a valid language identifier.
...
Object.defineProperty(Prism.languages.markup.tag, 'addInlined', {
	...
	value: function addInlined(tagName, lang) {
		...
		includedCdataInside['sourceCode' + lang] = {
			pattern: /(^<!\[CDATA\[)[\s\S]+?(?=\]\]>$)/i,
			lookbehind: true,
...
(function (Prism) {
	...
	// https://docs.microsoft.com/en-us/dotnet/csharp/sourceCodereference/keywords/
	var keywordKinds = {
	...
	// strings & characters
	// https://docs.microsoft.com/en-us/dotnet/csharp/sourceCodereference/sourceCodespecification/lexical-structure#character-literals
	// https://docs.microsoft.com/en-us/dotnet/csharp/sourceCodereference/sourceCodespecification/lexical-structure#string-literals
	...
	Prism.languages.csharp = Prism.languages.extend('clike', {
		...
		'keyword': keywords,
		// https://docs.microsoft.com/en-us/dotnet/csharp/sourceCodereference/sourceCodespecification/lexical-structure#literals
					...
					for (var i = 0; i < tokens.length; i++) {
						...
						if (typeof token === 'string' || (token.content && typeof token.content === 'string')) {
							...
							if (index > -1) {
								...
								var middle = new Prism.Token(language, Prism.tokenize(t, env.grammar), 'sourceCode' + language, t);
...
(function (Prism) {
	...
	var IDInside = {
		'markup': {
			pattern: /(^<)[\s\S]+(?=>$)/,
			lookbehind: true,
			alias: ['sourceCode markup', 'sourceCode html', 'sourceCod exml'],
			inside: Prism.languages.markup
		}
	};
		...
		if (Prism.languages[filter.language]) {
			all_filters['filter-' + filter.filter] = {
				...
				inside: {
					...
					'text': {
						pattern: /[\s\S]+/,
						alias: [filter.language, 'sourceCode' + filter.language],
...
(function (Prism) {
	...
	function tokenizeEmbedded(code, grammar, language) {
		...
		return new Prism.Token(language, embeddedTokens, 'sourceCode' + language, code);
	}
...
Prism.languages.log = {
	...
	'exception': {
		...
		lookbehind: true,
		greedy: true,
		alias: ['javastacktrace', 'sourceCodejavastacktrace'],
	...
	Prism.hooks.add('after-tokenize', function (env) {
		...
		function walkTokens(tokens) {
			...
			for (var i = 0, l = tokens.length; i < l; i++) {
				...
				/*
				 * Add the correct `sourceCodexxxx` class to this code block. Keep in mind that the `code-language` token
...
// https://docs.couchbase.com/server/current/n1ql/n1ql-sourceCodereference/index.html
...
Prism.languages.n1ql = {
	...
	'parameter': /\$[\w.]+/,
	// https://docs.couchbase.com/server/current/n1ql/n1ql-sourceCodereference/reservedwords.html#n1ql-reserved-words
...
Prism.languages.plsql = Prism.languages.extend('sql', {
	...
	// https://docs.oracle.com/en/database/oracle/oracle-database/21/lnpls/plsql-sourceCodefundamentals.html#GUID-96A42F7C-7A71-4B90-8255-CA9C8BD9722E
	'operator': /:=?|=>|[<>^~!]=|\.\.|\|\||\*\*|[-+*/%<>=@]/
});
...
// https://docs.microsoft.com/en-us/powerquery-m/power-query-m-sourceCodespecification
		...
		'group': [
			{
				...
				// https://docs.microsoft.com/en-us/dotnet/standard/base-types/regular-expression-sourceCodequick-reference?view=netframework-4.7.2#grouping-constructs
...
(function (Prism) {
	...
	function createT4(insideLang) {
		...
		var className = 'sourceCode' + insideLang;
